...
def dataarray_wrapper(func, grid):
    '''
    runs a function, and if the result is an xarray dataset, it creates an UgridDataset around it
    '''
    @wraps(func)
...
def dataset_wrapper(func, grid):
    '''
    runs a function, and if the result is an xarray dataset or an xarray dataArray, it creates 
    an UgridDataset or an UgridDataArray around it
    '''
    @wraps(func)
    def wrapped(*args, **kwargs):
        result = func(*args, **kwargs)
...
class UgridAccessor:
    ...
    def sel_points(self, points_x, points_y):
        '''
        returns subset of dataset or dataArray containing specific face indices. INput arguments are point coordinates. 
        The result dataset or dataArray contains only the faces containing these points.   
        '''
    ...
    def _sel_slices(self, x: slice, y:slice):
        if (x.start is None) or (x.stop is None) or (y.start is None) or (y.stop is None):
           ...
           raise ValueError("slices should both have a stepsize, or neither should have a stepsize") 
