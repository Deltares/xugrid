...
class Ugrid:
    ...
    def __init__(self,  ds: xr.Dataset ):
        ...
        mesh_2d_variables = self._get_values_with_attribute(ds, "cf_role", "mesh_topology")
        assert(len(mesh_2d_variables) == 1)
        self.mesh_2d = mesh_2d_variables[0]

        node_coord_array_names = self._get_topology_array_with_role( self.mesh_2d, "node_coordinates")
        connectivity_array_names = self._get_topology_array_with_role( self.mesh_2d, "face_node_connectivity")
    ...
    def remove_topology(self, obj: Union[xr.Dataset, xr.DataArray]):
       '''
       removes the grid topology data from a dataset. Use after creating an Ugrid object from the dataset
       '''
    ...
    def _get_topology_array_with_role(self, mesh_2d_variable: xr.core.dataarray.DataArray, role: str):
    ...
    def locate_faces(self, points) :
        '''
        returns the face indices in which the points lie. The points should be provided as an (N,2) array. The result is an (N) array
        '''
    ...
    def locate_faces_bounding_box(self, xmin, xmax, ymin, ymax):
        '''
        given a rectangular bounding box, this function returns the face indices which lie in it.
        '''
