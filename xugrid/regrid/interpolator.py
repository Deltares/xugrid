...
class Interpolator(BaseRegridder, abc.ABC):
    @property
    def weights(self):
...
class BarycentricInterpolator(Interpolator):
    """
    The BaryCentricInterpolator uses barycentric weights as interpolation
    weights between the nodes of a topology or between the centroids of faces.
    It finds by which source faces the centroid is surrounded (via its centroidal
    voronoi tesselation), and computes barycentric weights to interpolate smoothly
    between the values associated with the source faces.

    Classically, barycentric coordinates are defined for triangles. The
    BarycentricInterpolator uses Wachspress barycentric coordinates to support
    grids of convex polygons.
...
class InverseDistanceInterpolator(Interpolator):
    _JIT_FUNCTIONS = {"mean": BaseRegridder.make_regrid(reduce.mean)}
    ...
    def __init__(
        ...
        target,
        target_dim: Optional[str] = None,
        max_distance: Optional[float] = np.inf,
        min_points: int = 0,
        max_points: int = 12,
        power: float = 2.0,
        smoothing: float = 0.0,
    ...
    def _compute_weights(
        ...
        source_index, target_index, weights = source.inverse_distance(
            target,
            tolerance=tolerance,
            max_distance=max_distance,
            min_points=min_points,
