...
class TestRegridder:
    ...
    def test_check_source_target_types(self, disk, cls):
    ...
    def test_centroid_locator_regridder_structured(
        self, grid_data_a, grid_data_a_layered, grid_data_b, expected_results_centroid
    ):
        ...
        regridder = CentroidLocatorRegridder(
            source=grid_data_a_layered, target=grid_data_b
        )
        broadcasted = regridder.regrid(grid_data_a_layered)
        assert broadcasted.dims == ("layer", "y", "x")
        assert (
            broadcasted.fillna(0.0).isel(layer=0)
            == expected_results_centroid.fillna(0.0)
        ).any()

    def test_centroid_locator_regridder(self, disk, quads_1):
        square = quads_1
    ...
    def test_overlap_regridder_structured(
        self, grid_data_a, grid_data_a_layered, grid_data_b, expected_results_overlap
    ):
    ...
    def test_overlap_regridder(self, disk, quads_1):
        square = quads_1
    ...
    def test_linear_interpolator_structured(
        self, grid_data_a, grid_data_a_layered, grid_data_b, expected_results_linear
    ):
        ...
        # With broadcasting
        regridder = BarycentricInterpolator(
            source=grid_data_a_layered, target=grid_data_b
        )
        broadcasted = regridder.regrid(grid_data_a_layered)
        assert broadcasted.dims == ("layer", "y", "x")
    ...
    def test_barycentric_interpolator(self, disk, quads_0_25):
        square = quads_0_25
    ...
    def test_regridder_from_weights(self, cls, disk, quads_1):
        square = quads_1
    ...
    def test_regridder_from_weights_layered(self, cls, disk, disk_layered, quads_1):
        square = quads_1
        regridder = cls(source=disk, target=square)
        result = regridder.regrid(disk)
        weights = regridder.weights
        ...
        assert np.array_equal(
            new_result.sel(layer=1).values, result.values, equal_nan=True
        )
    ...
    def test_regridder_from_dataset(self, cls, disk, quads_1):
        square = quads_1
        regridder = cls(source=disk, target=square)
        result = regridder.regrid(disk)
    ...
    def test_regridder_daks_arrays(
        ...
        assert result.isel(layer=0).equals(
            grid_data_dask_expected_layered.isel(layer=0)
        )

    def test_create_percentile_method(self):
        with pytest.raises(ValueError):
    ...
    def test_barycentric_concave(self):
        vertices = np.array(
            [
                [0.0, 0.0],
                [3.0, 0.0],
    ...
    def test_barycentric_structured(self):
        da = xr.DataArray(
            data=[[1, 2], [3, 4]],
